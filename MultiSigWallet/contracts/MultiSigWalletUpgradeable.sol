 ...
 * @notice 可升级的多签钱包合约，使用Openzeppelin的Initializable
 */
contract MultiSigWalletUpgradeable is Initializable, ContextUpgradeable {
    ...
    modifier notConfirmed(uint256 txIndex) {
        require(
            !isConfirmed[txIndex][_msgSender()],
    ...
    function revokeConfirmation(
        uint256 txIndex
    ) public virtual onlyOwner txExists(txIndex) notExecuted(txIndex) {
        // 获取交易引用
        Transaction storage transaction = transactions[txIndex];
        // 验证：当前所有者必须已经确认过此交易
        require(
    ...
    function executeTransaction(
        ...
        // 重要：先标记为已执行（防止重入攻击）
        // 这是 Checks-Effects-Interactions 模式中的 Effects步骤
        transaction.executed = true;
        // 执行外部调用（Interactions步骤）
        // 向目标地址发送 ETH并调用函数（如果有data）
